    ...
    def load(self, machine, operator):
        ...
        with operator.request() as req:
            ...
            with open(EVENTS_PATH, 'a') as log:
                log.write(
                    f"{real_time.strftime('%Y-%m-%d %H:%M:%S')},{machine},{end.strftime('%Y-%m-%d %H:%M:%S')}, qualityCheck  occupied = {self.occupied} ready to unload = {self.is_ready_to_unload}\n")
            self.occupied = False
            #self.done_event = self.env.process(self._hold_part(machine))
            self.env.process(self._hold_part(machine, real_time))

    def _hold_part(self, machine, real_time):
        """
            Description:
